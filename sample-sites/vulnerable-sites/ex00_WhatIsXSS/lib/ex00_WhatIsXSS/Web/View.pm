package ex00_WhatIsXSS::Web::View;
use strict;
use warnings;
use utf8;
use Carp ();
...
sub make_instance {
    my ($class, $context) = @_;
    Carp::croak("Usage: ex00_WhatIsXSS::View->make_instance(\$context_class)") if @_!=2;
    ...
    unless (exists $view_conf->{path}) {
        ...
        if ( -d $tmpl_path ) {
            ...
            my $share_tmpl_path = eval { File::Spec->catdir(File::ShareDir::dist_dir('ex00_WhatIsXSS'), 'tmpl') };
            if ($share_tmpl_path) {
    ...
    my $view = Text::Xslate->new(+{
        ...
        'module'   => [
            ...
            'ex00_WhatIsXSS::Web::ViewFunctions',
        ],
        'function' => {
        },
